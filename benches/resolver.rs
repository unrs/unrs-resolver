...
use rayon::prelude::*;
use unrs_resolver::{FileSystem as FileSystemTrait, FileSystemOs, PackageJson};
...
fn resolve_options() -> unrs_resolver::ResolveOptions {
    use unrs_resolver::{AliasValue, ResolveOptions};
...
fn unrs_resolver_memory() -> unrs_resolver::ResolverGeneric<BenchMemoryFS> {
    use unrs_resolver::ResolverGeneric;
    let fs = BenchMemoryFS::new();
    ResolverGeneric::new_with_file_system(fs, resolve_options())
}

fn unrs_resolver_real() -> unrs_resolver::Resolver {
...
fn bench_resolver_memory(c: &mut Criterion) {
    ...
    for (path, request) in &data {
        assert!(
            unrs_resolver_memory().resolve(path, request).is_ok(),
            "{} {request}",
            path.display()
        );
    }
    ...
    for i in symlinks_range.clone() {
        assert!(
            unrs_resolver_memory().resolve(&symlink_test_dir, &format!("./file{i}")).is_ok(),
            "file{i}.js"
        );
    }
    ...
    group.bench_with_input(BenchmarkId::from_parameter("single-thread"), &data, |b, data| {
        let unrs_resolver = unrs_resolver_memory();
        b.iter(|| {
            for (path, request) in data {
    ...
    group.bench_with_input(BenchmarkId::from_parameter("drop"), &data, |b, data| {
        b.iter(|| {
            let unrs_resolver = unrs_resolver_memory(); // Measure `Drop` performance.
            for (path, request) in data {
                _ = unrs_resolver.resolve(path, request);
            }
        });
    ...
    group.bench_with_input(BenchmarkId::from_parameter("find tsconfig"), &data, |b, data| {
        ...
        let paths = data
            .iter()
            .map(|(path, request)| unrs_resolver.resolve(path, request).unwrap().into_path_buf())
            .collect::<Vec<_>>();
        b.iter(|| {
            for path in &paths {
                let _ = unrs_resolver.find_tsconfig(path);
            }
        });
    });
}
...
fn bench_resolver_real(c: &mut Criterion) {
    ...
    for (path, request) in &data {
        assert!(
            unrs_resolver_real().resolve(path, request).is_ok(),
            "{} {request}",
            path.display()
        );
    }
    ...
    for i in symlinks_range.clone() {
        assert!(
            unrs_resolver_real().resolve(&symlink_test_dir, &format!("./file{i}")).is_ok(),
            "file{i}.js"
        );
    }
    ...
    group.bench_with_input(BenchmarkId::from_parameter("single-thread"), &data, |b, data| {
        let unrs_resolver = unrs_resolver_real();
        b.iter(|| {
            for (path, request) in data {
                _ = unrs_resolver.resolve(path, request);
            }
...
mod memory_fs {
    ...
    use unrs_resolver::{FileMetadata, FileSystem, ResolveError};
    use walkdir::WalkDir;
